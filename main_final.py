...
def generate_forecast(res, group_eng, product, forecast_weeks, forecast_method,
    ...
    for i, f_date in enumerate(forecast_dates):
        ...
        history.append(history[-1])  # placeholder for current step
        for window in [4, 8, 12]:
            row[f'roll{window}_mean'] = np.mean(history[-(window+1):-1])
            row[f'roll{window}_std'] = np.std(history[-(window+1):-1]) if window > 1 else 0.0

        row['volume_trend'] = np.mean(history[-8:])
        row['volume_volatility'] = np.std(history[-4:]) if len(history) > 1 else 0.0
        row['volume_change'] = (history[-2] - history[-3]) / history[-3] if len(history) > 2 and history[-3] != 0 else 0.0
        ...
        history[-1] = forecast_val  # replace placeholder with actual forecast
