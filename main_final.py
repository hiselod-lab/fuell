...
def create_comprehensive_data_overview(df):
    ...
    def generate_tick_values(max_val: float) -> list:
        """Generate spaced tick values up to max_val."""
        tick_vals = [90_000_000, 100_000_000]
        for i in range(2, 10):
            val = i * 100_000_000
            if val <= max_val:
                tick_vals.append(val)
        billion = 1
        while billion * 1_000_000_000 <= max_val:
            tick_vals.append(billion * 1_000_000_000)
            billion += 1
        return tick_vals
    ...
    def create_product_chart(product_df, log_y=False):
        ...
        if log_y:
            ...
            tick_vals = generate_tick_values(max_val)
    ...
    with tab_product:
        ...
        show_lagged_product = st.toggle("Show lagged values", key="product_lagged")
        fig_product = create_product_chart(product_df, log_y=show_lagged_product)
        st.plotly_chart(fig_product, use_container_width=True)
    with tab_region_product:
        show_lagged_rp = st.toggle("Show lagged values", key="rp_lagged")
    ...
    show_lagged_month = st.toggle("Show lagged values", key="monthly_lagged")
