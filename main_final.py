...
def generate_direct_forecast(model_bundle, history, steps, include_confidence=True, confidence_level=0.95):
    """Generate forecasts without feeding predictions back into the feature set.

    This approximates a direct multi-step strategy where each horizon is
    predicted independently using the last known observations. Temporal
    features (e.g. seasonality) are updated for each step while lag-based
    ...
    base_history = history.copy().sort_values('week_start')
    if not pd.api.types.is_datetime64_any_dtype(base_history['week_start']):
        base_history['week_start'] = pd.to_datetime(base_history['week_start'])

    last_row = base_history.iloc[-1:].copy()
    forecasts, dates, lower, upper = [], [], [], []
    z_lookup = {0.99: 2.58, 0.95: 1.96, 0.9: 1.645, 0.8: 1.28}
    z = z_lookup.get(confidence_level, 1.96)
    ...
    for step in range(1, steps + 1):
        ...
        new_row['week_start'] = base_history['week_start'].iloc[-1] + timedelta(days=7 * step)
        tmp_history = pd.concat([base_history, new_row], ignore_index=True)
        eng = enhanced_feature_engineering(tmp_history, forecasting_mode=True)
        features = eng[feature_cols].iloc[-1:]
        ...
        if ensemble_method == 'Weighted Average':
            ...
            forecast = (pred_lgbm + pred_rf) / 2
        forecasts.append(forecast[0])
        dates.append(new_row['week_start'].iloc[0])
...
def generate_detailed_forecast(res, group_eng, product, forecast_weeks, forecast_method,
    ...
    """Generate forecasts using either a direct or recursive strategy."""

    if forecast_method == 'Direct':
        result = generate_direct_forecast(res, group_eng, forecast_weeks,
                                          include_confidence, confidence_level)
    else:
        result = generate_forecast(res, group_eng, forecast_weeks,
                                   include_confidence, confidence_level)

    result.update({
        'dates_str': [d.strftime('%Y-%m-%d') for d in result['dates']],
