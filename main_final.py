...
def generate_detailed_forecast(res, group_eng, product, forecast_weeks, forecast_method,
    ...
    """Generate forecasts using the trained model bundle.

    The previous implementation manually manipulated many features and injected
    randomness, which often produced flat or unstable predictions. This version
    leverages `generate_forecast` so that features are regenerated at each step
    and prior predictions feed back into the history, allowing the model to
    follow learnt trends.
    """
    ...
    if len(y_test) == len(y_pred) and len(y_test) > 1:
    ...
    history_cols = ['week_start', 'sales_volume'] + model_bundle['feature_cols']
    ...
    forecast['horizon'] = forecast_weeks
    forecast['confidence_level'] = confidence_level if include_confidence else None
    return forecast
