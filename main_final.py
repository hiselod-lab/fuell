...
def create_comprehensive_data_overview(df):
    ...
    def get_custom_ticks(max_val: float) -> list:
        """Generate custom tick positions: 90M, 100M, 200M..900M, 1B, 2B, ..."""
        ticks = [90_000_000]
        if max_val >= 100_000_000:
            ticks.append(100_000_000)
        for v in range(200_000_000, min(900_000_000, int(max_val)) + 1, 100_000_000):
            ticks.append(v)
        billion = 1_000_000_000
        while billion <= max_val:
            ticks.append(billion)
            billion += 1_000_000_000
        return ticks
    ...
    def create_product_chart(product_df, log_y=False):
        ...
        if log_y:
            ...
            tick_vals = get_custom_ticks(max_val)
            tick_text = [format_tick(v) for v in tick_vals]
            fig_product.update_yaxes(tickmode='array', tickvals=tick_vals, ticktext=tick_text)
    ...
    def create_region_product_chart(df, log_y=False):
        ...
        if log_y:
            ...
            tick_vals = get_custom_ticks(max_val)
            tick_text = [format_tick(v) for v in tick_vals]
            fig_rp.update_yaxes(tickmode='array', tickvals=tick_vals, ticktext=tick_text)
    ...
    with tab_product:
        ...
        show_lagged_product = st.toggle("Lagged values", key="lag_product")
        fig_product = create_product_chart(product_df, log_y=show_lagged_product)
        st.plotly_chart(fig_product, use_container_width=True)
    with tab_region_product:
        show_lagged_rp = st.toggle("Lagged values", key="lag_rp")
    ...
    def create_monthly_sales_chart(df, log_y=False):
        ...
        if log_y:
            ...
            tick_vals = get_custom_ticks(max_val)
            tick_text = [format_tick(v) for v in tick_vals]
            fig_monthly.update_yaxes(tickmode='array', tickvals=tick_vals, ticktext=tick_text)
        fig_monthly.update_xaxes(tickangle=45)
        return fig_monthly

    show_lagged_month = st.toggle("Lagged values", key="lag_month")
