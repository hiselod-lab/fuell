...
def enhanced_feature_engineering(df, forecasting_mode=False):
    """Create deterministic and random features for training or forecasting."""

    df = df.copy()

    # Price elasticity and volatility
    df['price_elasticity'] = np.where(
        df['avg_price'] != 0, df['sales_volume'] / df['avg_price'], np.nan
    )
    ...
    df['price_over_volume'] = np.where(
        df['sales_volume'] != 0, df['avg_price'] / df['sales_volume'], np.nan
    )

    # External factors
    if forecasting_mode:
        # During forecasting, keep external factors neutral
        df['weather_factor'] = 1.0
        df['economic_factor'] = 1.0
        df['event_factor'] = 1.0
    else:
        # Use a local RNG so global state isn't affected
        rng = np.random.default_rng(42)
        df['weather_factor'] = rng.normal(1, 0.1, len(df))
        df['economic_factor'] = rng.normal(1, 0.05, len(df))
        df['event_factor'] = rng.normal(1, 0.15, len(df))
...
def generate_forecast(
    ...
    """Generate forecasts for a region-product pair.

    Models are refitted on all available data, and future features are derived
    from the last observation without random perturbations. Optional confidence
    intervals use the standard deviation of validation residuals."""
    ...
    X_all = history[feature_cols]
    y_all = history["sales_volume"]
    X_all_clean = pd.DataFrame(imputer.fit_transform(X_all), columns=feature_cols, index=X_all.index)
    if feature_selection_enabled and selector is not None:
        X_all_sel = selector.fit_transform(X_all_clean, y_all)
    else:
        X_all_sel = X_all_clean.values
    lgbm.fit(X_all_sel, y_all)
    rf.fit(X_all_sel, y_all)

    residual_std = float((y_test - y_pred).std()) if len(y_test) == len(y_pred) and len(y_test) > 1 else metrics.get("RMSE", 0)
    z = NormalDist().inv_cdf(0.5 + confidence_level / 2) if include_confidence else 0
    ...
    for _ in range(forecast_weeks):
        ...
        source = history if forecast_method == "Recursive" else history
        base = source.iloc[-1:].copy()
        base["week_start"] = next_date
        base["month"] = next_date.month
        base["year"] = next_date.year
        base["week_of_year"] = next_date.isocalendar().week
        base["seasonal_sin"] = np.sin(2 * np.pi * base["week_of_year"] / 52)
        base["seasonal_cos"] = np.cos(2 * np.pi * base["week_of_year"] / 52)
        base["quarterly_sin"] = np.sin(2 * np.pi * base["week_of_year"] / 13)
        base["quarterly_cos"] = np.cos(2 * np.pi * base["week_of_year"] / 13)
        base["monthly_sin"] = np.sin(2 * np.pi * base["month"] / 12)
        base["monthly_cos"] = np.cos(2 * np.pi * base["month"] / 12)
        base["trend_factor"] = base["week_of_year"] / 52
        base["trend_squared"] = (base["week_of_year"] / 52) ** 2
        base["price_change"] = 0
        base["volume_change"] = 0
        base["weather_factor"] = 1.0
        base["economic_factor"] = 1.0
        base["event_factor"] = 1.0
        series = source["sales_volume"]
        base["volume_trend"] = series.rolling(8, min_periods=1).mean().iloc[-1]
        base["volume_volatility"] = series.rolling(4, min_periods=1).std().iloc[-1]
        base["regional_trend"] = series.rolling(12, min_periods=1).mean().iloc[-1]
        base["product_trend"] = base["regional_trend"]
        for window in [4, 8, 12]:
            base[f"roll{window}_mean"] = series.rolling(window, min_periods=1).mean().iloc[-1]
            base[f"roll{window}_std"] = series.rolling(window, min_periods=1).std().iloc[-1]

        X_f = base[feature_cols]
        X_f_clean = pd.DataFrame(imputer.transform(X_f), columns=feature_cols, index=X_f.index)
        if feature_selection_enabled and selector is not None:
            X_f_sel = selector.transform(X_f_clean)
        else:
            X_f_sel = X_f_clean.values
        pred_lgbm = lgbm.predict(X_f_sel)
        pred_rf = rf.predict(X_f_sel)
        pred = (lgbm_weight * pred_lgbm + (1 - lgbm_weight) * pred_rf) if ensemble_method == "Weighted Average" else (pred_lgbm + pred_rf) / 2
        pred = float(pred[0])
        forecasts.append(pred)
        dates.append(next_date)
        if include_confidence:
            margin = z * residual_std
            lower_bounds.append(max(0.0, pred - margin))
            upper_bounds.append(pred + margin)
        if forecast_method == "Recursive":
            base["sales_volume"] = pred
