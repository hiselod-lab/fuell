...
def generate_detailed_forecast(res, group_eng, product, forecast_weeks, forecast_method,
    ...
    """Generate forecasts using either direct or recursive strategy.

    The function reuses preprocessing objects and models stored in ``res`` and
    rebuilds deterministic features for each step via
    :func:`enhanced_feature_engineering`. Confidence intervals are derived from
    the standard deviation of residuals on the test set.
    """
    ...
    if lgbm is None or rf is None or imputer is None or not feature_cols:
        return {
            "dates": [],
            "dates_str": [],
            "values": [],
    ...
    if len(y_test) > 1 and len(y_test) == len(y_pred):
        residual_std = float(np.std(y_test - y_pred, ddof=1))
    else:
        residual_std = 0.0

    history_cols = ["week_start", "sales_volume", "Region", "Product", "avg_price"]
    if "week_of_year" in group_eng.columns:
        history_cols.append("week_of_year")
    if "month" in group_eng.columns:
        history_cols.append("month")
    history = group_eng[history_cols].copy()
    history["week_start"] = pd.to_datetime(history["week_start"])
    ...
    for _ in range(forecast_weeks):
        ...
        if forecast_method == "Recursive":
            ...
            history.loc[history.index[-1], "sales_volume"] = history["sales_volume"].iloc[-2]
