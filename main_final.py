...
def enhanced_feature_engineering(df, forecasting_mode=False):
    df = df.copy()
    # Ensure chronological order and a continuous time index per group
    if not pd.api.types.is_datetime64_any_dtype(df['week_start']):
        df['week_start'] = pd.to_datetime(df['week_start'])
    df = df.sort_values('week_start')
    df['time_index'] = df.groupby(['Region', 'Product']).cumcount()
...
def generate_forecast(res, group_eng, product, forecast_weeks, forecast_method,
    ...
    time_index = (
        int(last_row['time_index'].iloc[0])
        if 'time_index' in last_row.columns else len(group_eng) - 1
    )

    # history of sales for rolling features
    ...
    for i, f_date in enumerate(forecast_dates):
        ...
        # Rolling statistics based on historical and predicted values
        for window in [4, 8, 12]:
            row[f'roll{window}_mean'] = np.mean(history[-window:])
            row[f'roll{window}_std'] = np.std(history[-window:]) if len(history[-window:]) > 1 else 0.0

        row['volume_trend'] = np.mean(history[-8:])
        row['volume_volatility'] = np.std(history[-4:]) if len(history[-4:]) > 1 else 0.0
        row['volume_change'] = (
            (history[-1] - history[-2]) / history[-2]
            if len(history) > 1 and history[-2] != 0 else 0.0
        )
        row['regional_trend'] = np.mean(history[-12:])
        row['product_trend'] = np.mean(history[-12:])

        # Preserve last known values for price-based features
