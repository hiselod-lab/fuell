...
def create_comprehensive_data_overview(df):
    ...
    def get_milestone_ticks(max_val: float) -> list[float]:
        """Return milestone tick positions up to max_val."""
        ticks = [v for v in [9e7, 1e8, 2e8, 3e8, 4e8] if v <= max_val]
        current = 1e9
    ...
    def create_product_chart(product_df, log_y=False):
        ...
        if log_y:
            ...
            tick_vals = get_milestone_ticks(max_val)
