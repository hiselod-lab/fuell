...
def generate_forecast(res, group_eng, product, forecast_weeks, forecast_method,
    ...
    for i, f_date in enumerate(forecast_dates):
        ...
        # Rolling statistics based purely on the available history
        for window in [4, 8, 12]:
            recent = history[-window:] if len(history) >= window else history
            row[f'roll{window}_mean'] = np.mean(recent)
            row[f'roll{window}_std'] = np.std(recent) if len(recent) > 1 else 0.0

        row['volume_trend'] = np.mean(history[-8:]) if len(history) >= 8 else np.mean(history)
        row['volume_volatility'] = (
            np.std(history[-4:]) if len(history) >= 4 else (np.std(history) if len(history) > 1 else 0.0)
        )
        row['volume_change'] = (
            (history[-1] - history[-2]) / history[-2]
            if len(history) > 1 and history[-2] != 0
            else 0.0
        )
        row['regional_trend'] = np.mean(history[-12:]) if len(history) >= 12 else np.mean(history)
        row['product_trend'] = np.mean(history[-12:]) if len(history) >= 12 else np.mean(history)
