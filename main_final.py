...
def generate_detailed_forecast(res, group_eng, product, forecast_weeks, forecast_method,
    ...
    """Generate forecasts with updated features and optional confidence intervals."""
    ...
    residual_std = res.get('residual_std', 0)
    ...
    for step in range(forecast_weeks):
        if forecast_method == 'Direct':
            temp_history = base_history.copy()
        else:
        ...
        forecasts.append(forecast_val[0])
        if include_confidence:
            margin = z * residual_std * np.sqrt(step + 1)
            lower_bounds.append(forecast_val[0] - margin)
            upper_bounds.append(forecast_val[0] + margin)
        else:
            lower_bounds.append(None)
            upper_bounds.append(None)

        if forecast_method == 'Recursive':
            temp_history.loc[temp_history.index[-1], 'sales_volume'] = forecast_val[0]
            history = temp_history

    forecast_dates = [base_history['week_start'].iloc[-1] + timedelta(days=7 * (i + 1)) for i in range(forecast_weeks)]
            ...
            if st.session_state.get('run_region_fuel', False):
                ...
                with st.spinner("Training region-fuel models..."):
                    ...
                    for idx, ((region, product), group) in enumerate(combinations):
                        ...
                        # Refit preprocessing and models on the full dataset
                        imputer_full = SimpleImputer(strategy='mean')
                        X_full = group_eng[feature_cols]
                        X_full_clean = pd.DataFrame(imputer_full.fit_transform(X_full), columns=feature_cols, index=group_eng.index)

                        if feature_selection_enabled:
                            selector_full = SelectKBest(score_func=f_regression, k=min(k_features, len(feature_cols)))
                        ...
                        result_data = {
                            ...
                            'imputer': imputer_full,
                            'selector': selector_full if feature_selection_enabled else None,
                            'feature_cols': feature_cols,
