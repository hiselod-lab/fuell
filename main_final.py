...
def create_comprehensive_data_overview(df):
    ...
    # Basic statistics
    # Display metrics in 3 columns instead of 4 to give more space
    ...
    # Display date range as markdown text instead of a metric to avoid truncation
    st.markdown(f"**Date Range:** {stats['date_range']}")
        
    # Business insights for directors/managers
    st.markdown("### 📈 Business Insights for Management")
    col1, col2 = st.columns(2)
    
    with col1:
        # Weekly growth rates
        st.markdown("#### Week-over-Week Growth by Product")
        weekly_data = df.groupby(['week_start', 'Product'])['sales_volume'].sum().reset_index()
        weekly_data = weekly_data.sort_values(['Product', 'week_start'])
        
        # Calculate week-over-week growth for each product
        growth_data = []
        for product in weekly_data['Product'].unique():
            product_data = weekly_data[weekly_data['Product'] == product].copy()
            product_data['prev_volume'] = product_data['sales_volume'].shift(1)
            product_data = product_data.dropna()
            product_data['wow_growth'] = ((product_data['sales_volume'] - product_data['prev_volume']) / product_data['prev_volume'] * 100)
            
            # Get the latest week's growth
            latest_growth = product_data.iloc[-1]['wow_growth'] if len(product_data) > 0 else 0
            latest_week = product_data.iloc[-1]['week_start'] if len(product_data) > 0 else 'N/A'
            growth_data.append({
                'Product': product,
                'Latest Week': latest_week,
                'WoW Growth (%)': round(latest_growth, 2),
                'Latest Volume': format_value_with_unit(product_data.iloc[-1]['sales_volume']) if len(product_data) > 0 else 'N/A'
            })
    ...
    with col2:
        ...
        region_performance = df.groupby('Region')['sales_volume'].sum().reset_index()
        region_performance = region_performance.sort_values('sales_volume', ascending=False).head(5)
        region_performance['Sales Volume (Litres)'] = region_performance['sales_volume'].apply(format_value_with_unit)
        region_performance = region_performance[['Region', 'Sales Volume (Litres)']]
    ...
    sales_tabs = st.tabs(["By Region", "By Product", "Heatmap"])
    ...
    with sales_tabs[1]:
        fig_product = create_product_volume_chart(df)
    ...
    def create_monthly_sales_chart(df):
        ...
        # Monthly sales trend by product (PMG, HSD, HOBC)
        df_monthly = df.groupby([df['week_start'].dt.to_period('M'), 'Product'])['sales_volume'].sum().reset_index()
    ...
    # Region-Product heatmap section removed as it's now part of the tabbed interface
